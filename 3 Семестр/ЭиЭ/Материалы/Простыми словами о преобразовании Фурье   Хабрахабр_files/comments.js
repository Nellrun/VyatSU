		...
		$.post('/json/vote/',{ti:target_id, tt:target_type, pti: post_target_id, v:vote},function(json){	 
			if(json.messages == 'ok'){					
				...
				if( json.sign == 'positive' ){
  				$('#comment_'+target_id+' .message').removeClass('bad bad1 bad2 bad3 bad4 bad5')
				}else{
  				if( !$('#comment_'+target_id+' .message').hasClass('bad') ){
    				$('#comment_'+target_id+' .message').addClass('bad bad1');
  				}
				}
